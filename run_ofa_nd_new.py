...
from OFA_mbv3_extended.run_manager import DatasetRunConfig, SingleExitRunManager, EarlyExitRunManager
from OFA_mbv3_extended.train.utils import get_paths_dict, assign_phases_args, assign_dataset_args, assign_run_args, get_previous_path
...
if __name__ == "__main__":
    ...
    if args.task == "full":
        ...
        net = get_teacher_by_name(args.network)(
    ...
    # otherwise create the dynamic network
        ...
        if args.kd_ratio > 0:
            args.teacher_model = initialize_dyn_net(run_config.data_provider.n_classes, args)
            args.teacher_path = get_previous_path(args, paths_dict)
            init = torch.load(args.teacher_path, map_location="cpu")["state_dict"]
            args.teacher_model.load_state_dict(init)

            # get extract maximal network
            args.teacher_model.set_max_net()
            if "EE" not in args.network:
                maximal_subnet = args.teacher_model.get_active_subnet(preserve_weight=True)
            else:
                maximal_subnet = args.teacher_model.get_all_exits_subnet(preserve_weight=True)

            sub_train_size = run_config.data_provider.subset_size
            sub_train_batch_size = run_config.data_provider.subset_batch_size
            sdl = run_config.data_provider.build_sub_train_loader(sub_train_size, sub_train_batch_size)
            maximal_subnet.train()
            set_running_statistics(maximal_subnet, sdl)

            args.teacher_model = maximal_subnet
    ...
    if args.task == "full":
        ...
        if args.task == "kernel":
            ...
            if run_manager.start_epoch == 0:
                ...
                dynamic_net = run_manager.net.module if isinstance(run_manager.net,
                                                                   nn.DataParallel) else run_manager.net
                load_models(run_manager, dynamic_net, args.ofa_checkpoint_path)
                ...
                log_str += valid_log
                run_manager.write_log(log_str, "valid")
        ...
        # train elastic net-width
            ...
            if "P" not in args.network:
                ...
                train_elastic_net_width(train, run_manager, args, validate_func_dict)
        ...
        # train elastic net-depth
            ...
            if "EE" not in args.network:
                ...
                train_elastic_net_depth(train, run_manager, args, validate_func_dict)
    ...
    if args.task == "full":
        ...
        if isinstance(run_manager, SingleExitRunManager):
            img_size, val_loss, val_acc, val_acc5 = run_manager.validate_all_resolution(is_test=True)
        ...
        (subnets_losses_mean, subnets_top1_mean, subnets_top5_mean, valid_log) = validate(run_manager, is_test=True,
                                                                                          **validate_func_dict)
