...
class STL10DataProvider(DataProvider):
    ...
    def save_path(self):
        if self._save_path is None:
            self._save_path = 'running_experiment/datasets/private/STL10'  # home server
    ...
    #     if isinstance(image_size, list):
    #         resize_transform_class = MyModRandomResizedCrop
    #         print('Use MyModRandomResizedCrop: %s, \t %s' % MyModRandomResizedCrop.get_candidate_image_size(),
    #               'sync=%s, continuous=%s' % (MyModRandomResizedCrop.SYNC_DISTRIBUTED, MyModRandomResizedCrop.CONTINUOUS))
