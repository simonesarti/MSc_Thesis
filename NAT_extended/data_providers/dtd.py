...
class DTDDataProvider(DataProvider):
    ...
    def save_path(self):
        if self._save_path is None:
            self._save_path = 'running_experiment/datasets/private/dtd'
    ...
    def train_dataset(self, _transforms):
        dataset = torchvision.datasets.DTD(root=self.train_path, split="train", download=True, transform=_transforms)
        return dataset

    def valid_dataset(self, _transforms):
        dataset = torchvision.datasets.DTD(root=self.valid_path, split="val", download=True, transform=_transforms)
        return dataset

    def test_dataset(self, _transforms):
        dataset = torchvision.datasets.DTD(root=self.test_path, split="test", download=True, transform=_transforms)
        return dataset
    ...
    def build_train_transform(self, image_size=None, print_log=True, auto_augment='rand-m9-mstd0.5'):
        ...
        aa_params = dict(
            ...
            img_mean=tuple([min(255, round(255 * x))
                            for x in [0.5329876098715876, 0.474260843249454, 0.42627281899380676]]),
        )
