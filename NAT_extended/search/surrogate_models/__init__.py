...
class SurrogateModel:
    ...
    def _fetch(self, **kwargs):
        if self.method == 'rbf':
            ...
            return LGBMRegressor(
                objective='huber',
                n_jobs=kwargs["n_cores"]
            )

        elif self.method == 'catboost':
            from catboost import CatBoostRegressor
            return CatBoostRegressor(
                thread_count=kwargs["n_cores"]
            )
    ...
    def fit(self, inputs, targets, ensemble=False, pretrained=None, n_cores=1):
        if ensemble:
            return self.cross_validation(inputs, targets, pretrained, n_cores=n_cores)
        else:
            return self._fit(inputs, targets, pretrained, n_cores=n_cores)
    ...
    def cross_validation(self, inputs, targets, pretrained=None, seed=25130907, num_folds=10, n_cores=1):
        reset_seed(seed)
        perm = np.random.permutation(len(targets))

        pool, kendall_tau = [], []

        for i, test_split in enumerate(np.array_split(perm, num_folds), 1):

            train_split = np.setdiff1d(perm, test_split, assume_unique=True)

            if pretrained:
                expert = self._fit(inputs[train_split, :], targets[train_split], pretrained=pretrained[i], n_cores=n_cores)
            else:
                expert = self._fit(inputs[train_split, :], targets[train_split], n_cores=n_cores)

            pred = self.predict(expert, inputs[test_split, :])
            rmse, r, rho, tau = get_correlation(pred, targets[test_split])

            print("Fold {}: rmse = {:.4f}, pearson = {:.4f}, spearman = {:.4f}, kendall = {:.4f}"
                  .format(i, rmse, r, rho, tau))

            kendall_tau.append(tau)
            pool.append(expert)

        print("{}-fold KTau performance = {:.4f}({:.4f})".format(num_folds, np.mean(kendall_tau), np.std(kendall_tau)))
